...
- **Known limit, deliberately left.** Brand options (`sira_office_locations`, `sira_brand_tagline`, `sira_brand_address`) are one set per tenant with no locale, so the office list on `/ar/contact/` and the footer tagline and address stay in English. Localizing them needs a backend field (a per-locale brand options group) and a plugin deployment, which this decision does not authorize.

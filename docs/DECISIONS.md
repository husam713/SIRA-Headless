...
## ADR-034 - Arabic is a first-class locale inside the same Digital tenant (resolves 2C4-B09 for Digital)

- **Status:** Owner decision, 2026-09-08. This resolves the `2C4-B09` owner-decision requirement **for the SIRA Digital tenant**. It does not decide the model for the other five tenants, which remain English-only in practice and unaffected by this record.
- **Decision:** SIRA Digital serves English and Arabic from ONE WordPress Multisite site. There is no second Arabic site, and no network-wide translation plugin is imposed on tenants that did not ask for one. English is the default public locale; Arabic is reached through the `/ar/...` prefix. The two languages are independently editable and independently publishable.
- **What the previous documents said, and what now supersedes them.** `docs/SIRA-CMS-EDITORIAL-SYSTEM-ARCHITECTURE.md` §WS8 recorded `2C4-B09` as an owner decision gate with three candidate models and forbade authoring Arabic content before that decision. `docs/SIRA-EDITORIAL-ARCHITECTURE-SPEC.md` §18 forbade inventing locale routes or translated-record ownership and blocked `hreflang`. **Both prohibitions are lifted for the `digital` tenant only, by this record.** They remain in force everywhere else. The selected model is closest to WS8's model 3 (per-record translation inside `sira-core`) and explicitly rejects models 1 and 2 for this tenant.
- **Locale is stored explicitly, not inferred from a slug.** Every localizable record carries `sira_locale` (`en` or `ar`) as post meta, or term meta for taxonomy terms. A slug naming convention was considered and rejected as the source of truth: slugs are editor-editable, and a rename would silently move a record between languages. The `/ar/` URL prefix is a routing decision; `sira_locale` is the content decision; the two are deliberately not the same mechanism.
- **Translation pairing is explicit and one-directional.** A non-default-locale record may carry `sira_translation_of`, the database ID of its default-locale counterpart. The link is stored on the translation, so exactly one record owns it and there is no pair to keep symmetric. A translation with no counterpart is legal — an Arabic page that exists only in Arabic is a real editorial state, not an error.
- **Missing metadata degrades, it does not break.** A record with no `sira_locale` is treated as the default locale, with a slug-prefix fallback for records created before the field existed. An untranslated index falls back to the language it does exist in rather than rendering an empty page, because a content gap should read as a content gap and not as a broken build.
- **Arabic is not the English type system mirrored.** Latin negative tracking and sub-1.0 leading do not transfer: Arabic letterforms join, and letter-spacing severs those joins. Tracking is neutralised and leading relaxed wherever the document language is Arabic, with `lang="en"` islands keeping their own treatment. Numerals stay Latin, because mixing numeral systems within one page is worse than either alone. Only semantically directional interactions are mirrored; a range input is a magnitude axis and is left to the browser.
- **What this decision does NOT authorize:** machine translation as content ownership, a second WordPress site, a network-wide plugin activation, Arabic on the other five tenants, deployment, or DNS.

## ADR-035 - Digital launches on a Group subdomain and moves to the Saudi apex by configuration

- **Status:** Owner decision, 2026-09-08. Amends the hostname premise of ADR-033. ADR-033 is **not** reversed: `sirahdigital.sa` remains the intended production domain.
- **Decision:** For the current pre-launch stage, SIRA Digital's public frontend hostname is `digital.siratrgroup.com`. `sirahdigital.sa` is the future production hostname and is **not active**. Canonical URLs, `hreflang`, Open Graph, the sitemap and alias redirects all follow whichever hostname is ACTIVE, so today they name the subdomain.
- **Why the registry distinguishes "planned" from "alias".** An alias is another spelling of the current address; a planned hostname is the next address. Collapsing them would have made `sirahdigital.sa` canonical in metadata while it resolves nowhere, which is how a site ends up publishing canonical URLs that 404. `SiteDefinition` therefore gained `plannedCanonicalHostname` and `plannedAliases`, and the planned hostnames are registered now as redirect aliases so that the day DNS points at us they land correctly instead of 421-ing.
- **The cutover is configuration, not a migration.** `SIRA_CANONICAL_HOSTNAMES_JSON={"digital":"sirahdigital.sa"}` promotes the planned hostname: it becomes canonical, brings its own aliases with it, and `digital.siratrgroup.com` demotes to a redirect alias so no indexed URL is orphaned. No rebuild, no content move, no second deployment. A promotion target must be a hostname that site already declares, so a typo in an environment variable fails loudly instead of silently repointing a company.
- **Public hostname and CMS origin stay independent.** The WordPress origin is configured per tenant through `SIRA_WP_<TENANT>_GRAPHQL_URL` and is never derived from the public hostname. WordPress remains on Hostinger while the public Next.js frontend runs separately, and the public hostname does not expose `/wp-admin`. `tests/contract/digital-tenant-topology.test.ts` asserts this separation so a later change cannot quietly couple them.
- **The CMS origin moved on 2026-09-08, by owner decision.** Blog 6 was created on `sirahdigital.sa`, which put the WordPress site on the exact domain the public frontend is meant to take at cutover — the two cannot both hold one hostname. The owner chose to move the CMS to `digital.siratrgroup.com`, matching the four branch tenants. Executed with `update_blog_details()` and `update_blog_option()` rather than a raw `UPDATE`, after a verified 7.59 MB / 155-table backup, followed by a scoped `search-replace` across `wp_6_*`. Post GUIDs were rewritten too: the usual advice against that protects feed subscribers from duplicate items, and this tenant is pre-launch with `blog_public 0` and has never been publicly served, so there are none.
- **OPEN — the pre-launch hostname is now claimed twice.** WordPress answers on `digital.siratrgroup.com`, and that is also where the public frontend is meant to be served before cutover. One hostname cannot serve both. Nothing is broken today because no deployment is authorized, but this must be resolved before the frontend is deployed there. At cutover the frontend takes `sirahdigital.sa` and the conflict ends by itself.
- **The active topology is single-apex again, and says so.** `canonicalPublicProductionTopology` records `multiApex: false` with `plannedApexes: ["sirahdigital.sa"]` and `multiApexPlanned: true`. ADR-033's expansion is preserved as the end state rather than asserted as the present one.
- **What this decision does NOT authorize:** public DNS changes, domain registration, the WordPress CMS-domain migration, or production deployment. The Digital WordPress site must not be deleted or lost; it is blog 6 and it holds the tenant's content.

## ADR-036 - CMS origins relocated to `cms-<tenant>.siratrgroup.com` for the five non-Group tenants

- **Status:** Recorded 2026-09-10, after the fact. Executed the same day under direct real-time owner authorization in a bare SSH session against live production WordPress. It was **not** pre-authorized through a Task Packet, and it went beyond the bounded `cmsMutationAuthorization` scope in `project-state.json` (editorial seeding + Batch A taxonomy). This record reconciles the durable-state carriers and the topology contract test to that executed reality, in the manner of SOT-001 and SOT-002. It is closed as `SOT-003`.
- **Decision:** The WordPress Multisite CMS origin for `consulting`, `healthcare`, `realestate`, `lifestyle` and `digital` was relocated from `<tenant>.siratrgroup.com` to a dedicated `cms-<tenant>.siratrgroup.com` subdomain. **Group — blog 1, the network main site — was deliberately excluded** and stays on `siratrgroup.com`.
- **Why.** The plain `<tenant>.siratrgroup.com` subdomains are needed as the public frontend hostnames. A dedicated `cms-` subdomain per tenant separates the CMS origin from the public hostname at the DNS layer, which is the ADR-035 principle applied to every tenant rather than just Digital. It also, verified from a developer machine, is **not** subject to the ADR-032 CDN challenge that the plain tenant subdomains were returning — which is the condition that opened this work.
- **What was done, per tenant.** A fresh verified `mysqldump` immediately before each write; scoped `wp search-replace` across that tenant's own tables with `--all-tables-with-prefix` (so the SEO plugin's indexable tables are not silently skipped) and `--skip-columns=guid` for the four live tenants; the routing row updated through `wp_update_site()`, never a bare `UPDATE`; every dry-run count reconciled exactly against an independent SQL row count before writing. 567 replacements across the five tenants, every one an exact match. `guid` was rewritten only for Digital, which is pre-launch and has never been publicly served. Full redacted evidence: `artifacts/migration-blog6-redacted.md`.
- **This resolves the ADR-035 collision.** Moving the Digital WordPress origin off `digital.siratrgroup.com` is precisely what frees that hostname for the frontend. `openGates.digitalCmsOriginHostnameCollision` moves from OPEN to CLOSED. `authorization.digitalTenant.cmsOriginHostname` becomes `cms-digital.siratrgroup.com`.
- **Group is deferred, with reason.** Relocating the network main site also means editing `DOMAIN_CURRENT_SITE` in `wp-config.php` and the `wp_site` network-identity row, neither of which has a dry run, and it has no safe intermediate state — the instant WordPress stops claiming `siratrgroup.com`, requests still routed there get a hard error. `siratrgroup.com` currently serves the live legacy Group site, which ADR-025 (Group Staging First) requires kept live as the rollback target. The main-site relocation therefore belongs in the same window as the frontend production cutover, not before it. Tracked as `openGates.groupCmsOriginRelocation`.
- **Consequences for the repository.** Every deployment environment's `SIRA_WP_<non-Group>_GRAPHQL_URL` must name the `cms-` host over HTTPS; `frontend/.env.local` was updated 2026-09-10 and `frontend/.env.example` is updated here. `frontend/tests/contract/digital-tenant-topology.test.ts` is reworked so it asserts the collision is resolved rather than enforcing the pre-move values. The five previous subdomains are parked — they return the WordPress unknown-site signup redirect — until DNS points them at the frontend.
- **What this decision does NOT authorize:** the Group / blog-1 relocation, any DNS change, production deployment or cutover, deleting the parked subdomains or their DNS, or treating the migration as pre-authorized for any future CMS mutation. ADR-032's deploy-platform egress question is unchanged: `openGates.platformEgressReachabilityUnverified` stays OPEN; only local-dev egress to the `cms-` hosts has been verified.

## ADR-037 - Arabic is a first-class locale on Group and the four companies (resolves 2C4-B09 for all six tenants)

- **Status:** Owner decision, given in-session on 2026-09-17 ("ابدأ العربية") and reaffirmed on 2026-09-18 ("ابدأ"). Extends ADR-034 from the Digital tenant to Group, Consulting, Healthcare, Real Estate and Lifestyle. `2C4-B09` is thereby decided for every tenant of the network.
- **Decision:** The five remaining tenants serve English and Arabic from their existing WordPress sites on ADR-034's model, unchanged: English is the default public locale, Arabic is reached through the `/ar/…` prefix, every localizable record carries `sira_locale`, a translation carries `sira_translation_of`, a custom-post translation takes the `ar-` slug prefix, an Arabic page hangs under the `ar` page, and the `ar` page is the Arabic homepage. No second site, no translation plugin. `localeRoutesApproved` is `true` on all six tenants, so `hreflang`, the sitemap and the edge-purge planner now name both languages for every tenant.
- **What the frontend adds for it.** A translation's WordPress URI carries its locale in the slug (`/projects/ar-sira-prime/`); the public page does not (`/ar/projects/sira-prime/`). `frontend/src/lib/i18n/record-href.ts` is the one place that rule lives, and the project archive, the homepage chapters and the sitemap go through it. The project archive reads `siraLocale` and filters per request language with the same fallback the Digital indexes use; the project route tries the translation before the original. Names that live in English in the registry, the brand presets and the `sira_business_unit` taxonomy are said in Arabic by the chrome (`siteNames`, `businessUnits`, `companyStatus`).
- **How the content was written.** `tools/seed/atlas-seed-ar.php` with `tools/seed/atlas-seed.ar.json`, once per tenant, after a verified `mysqldump` (`~/sira-backups/sira-multisite-db-20260917T202219Z-pre-arabic.sql.gz`). A translation borrows its English twin's featured image, gallery, business unit, order and date; the Arabic homepage is a meta-level copy of the English homepage with every relationship re-pointed at the translations and the authored words laid over it. Production content, no `_sira_seed` marker — the owner's review is the gate, as for the English Atlas content (`docs/tasks/atlas-production-content.md`). Record: `docs/tasks/atlas-arabic-content.md`.
- **Brand options, closed 2026-09-18.** The brand options were one set per tenant with no locale, so the office list, footer tagline and address read in English on Arabic pages. Under the owner's same-day instruction to make the site ready, the plugin gained `sira_brand_tagline_ar`, `sira_brand_address_ar` and per-office `name_ar`, `address_ar`, `business_unit` (the office's company, for its accent), plus a curated `services` chapter on the branch homepage; the four files were linted and installed on the origin with timestamped rollback copies and the schema recaptured. `getBrand(siteKey, "ar")` folds the Arabic columns in.
- **What this decision does NOT authorize:** machine translation as content ownership, backend changes, deployment, DNS, merge to `main`, or touching the newsroom. The newsroom stays English-only; on an Arabic page its items are listed as written, with Arabic kickers.

## ADR-038 - Group's CMS origin relocated to `cms-group.siratrgroup.com`; the frontend deployed to Cloud Run; the apex handed to DNS

- **Status:** Executed 2026-09-18 under the owner's in-session authorization ("do everything you can, I agree to everything"), with the owner reserving the DNS change. Closes `openGates.groupCmsOriginRelocation` (the deferred half of ADR-036 / SOT-003) and prerequisite A of `docs/tasks/group-production-cutover-plan.md`.
- **Blog 1 moved.** After a verified `mysqldump` (`~/sira-backups/sira-multisite-db-20260918T105030Z-pre-group-relocation.sql.gz`, 7.7 MB, 156 tables): the `wp_site` row and network `siteurl`, the `wp_blogs` row and blog 1's `siteurl`/`home` (via `update_blog_details` / `update_option`), `DOMAIN_CURRENT_SITE` in `wp-config.php` (`.bak-20260918T105030Z` beside it), and a `search-replace` over the enumerated blog-1 tables only — never a prefix glob, `guid` kept — 66 replacements across `wp_options`, `wp_postmeta`, `wp_posts`; zero old-host occurrences remain outside `guid`. Rows 2–6 of `wp_blogs` untouched. Network Admin answers at `cms-group.siratrgroup.com/wp-admin/network/`; `/graphql` on `cms-group` returns the Group brand and the Arabic homepage.
- **The apex keeps answering — the plan's "no safe intermediate state" is closed by design, not by speed.** `wp-content/sunrise.php` maps a request for `siratrgroup.com` / `www.siratrgroup.com` to blog 1, so the legacy Bricks site (ADR-025's rollback target) still renders at the apex until DNS moves, and any request that reaches the origin for the apex afterwards is still served. `SUNRISE` is defined in `wp-config.php`.
- **Deployment.** The frontend image is built by Cloud Build from `main` (`ed781fdc` → `europe-west1-docker.pkg.dev/project-9ccb9553-1ed5-4945-88a/sira/nextjs:ed781fdc`) and serves the Cloud Run service `sira-frontend` (europe-west1, revision `00006-ts6`) with `SIRA_WP_GROUP_GRAPHQL_URL` on `cms-group` and `SIRA_SEARCH_INDEXING=off`. The four company subdomains were already mapped by the owner; `siratrgroup.com` and `www.siratrgroup.com` domain mappings now exist and wait for DNS. ADR-032's egress concern does not bite Cloud Run: every tenant renders with `data-brand-source="wordpress"` from the deployed service.
- **What stays with the owner:** the apex DNS records (A/AAAA to the Cloud Run mapping, `www` CNAME to `ghs.googlehosted.com.`), the final human review of the content on the live site, and the moment `SIRA_SEARCH_INDEXING` turns on. `blog_public = 0` is correct for every `cms-*` origin and stays: the public site's indexing is the frontend's switch, and the origins must not be indexed.
- **What this decision does NOT authorize:** deleting the legacy Bricks content (it travels with blog 1 at `cms-group`), removing the sunrise mapping before the stabilization period ends, or touching Digital (blog 6), whose 94 placeholder records are still its own gate.
